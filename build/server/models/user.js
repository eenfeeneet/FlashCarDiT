...
module.exports = (dbPoolInstance) => {
  ...
  let checkUserName = (username, callback) => {
    let query = `SELECT EXISTS (SELECT * FROM users WHERE user_name= $1)`;
    let values = [username];
  ...
  let registerNewUser = (user, callback) => {
    ...
    let query = 'INSERT INTO users (first_name, last_name, user_name, initials, password,account_created) VALUES ($1,$2,$3,$4,$5,now()) RETURNING *';
    let values = [user.firstname, user.lastname, user.username, user.initials, user.password];
  ...
  let setLogInDate = (username, callback) => {
    let query = 'UPDATE users SET last_login = now() WHERE user_name = $1 RETURNING *';
    let value = [username];
  ...
  let getUserId = (username, callback) => {
    const query = `SELECT id FROM users WHERE user_name = $1`;
  ...
  let getUserDetails = (username, callback) => {
    const query = `SELECT id, user_name, password, last_login, TO_CHAR(last_login :: DATE, 'Mon dd yyyy') lastlogin, TO_CHAR(account_created :: DATE, 'Mon dd yyyy') created  FROM users WHERE username = $1`;
    const values = [username];
